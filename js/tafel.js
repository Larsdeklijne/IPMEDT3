...
window.onload = function() {
  ...
  handhamer = '<a-entity class="hand" id="handhamer" scale="0.1 0.1 0.1" position="0.5 -0.4 -0.8" rotation="30 180 0" obj-model="obj: #hammer-obj; mtl: #hammer-mtl"></a-entity>';
    ...
    // objecten worden aangemaakt die later in de functie 'setTafelStappen' in een array worden geplaatst
    ...
    // objecten worden aangemaakt die later in de functie 'setKastStappen' in een array worden geplaatst
...
// functie die ervoor zorgt dat de cursor laad
function loadCursor(){
    document.getElementById('cursor-loader').setAttribute('material', 'color: lightgreen');
}

// functie die ervoor zorgt de cursor weggaat zodat er geen objecten geselecteerd meer kunnen worden
function quitCursor(){
    document.getElementById('cursor-loader').setAttribute('material', 'color:#3b4f70');
}

// functie die ervoor zorgt dat de hamer en schroevendraaier opgepakt worden en in de camera worden geplaatst
...
// functie die ervoor zorgt dat als de gebruiker richt op 'leeg hand' dat het object dat zit in de camera verwijderd worden
function emptyHand(){
    if(hand != '') {
        loadCursor();
        var timer = setTimeout(function () {
...
// functie die ervoor zorgt dat als de gebruiker een deuvel, schroef of ander onderdeel selecteert het geselecteerde onderdeel in de hand wordt geplaatst
...
// functie die ervoor zorgt dat alle benodigde objecten in beeld komen
function laadTafel()
{
  loadCursor();
  var timer = setTimeout(function () {
...
// functie die op basis van welke stap het is de bijbehorende acties uitvoerd.
...
// functie die het tafelblad plaatst
...
// functie die de tafelpoot plaatst
...
// functie die de schroef plaatst
    ...
    if(stepsTaken < 7 || (stepsTaken > 8 && stepsTaken <17) || (stepsTaken >= 20 && kastDeuvelsVerplaatst >= 12 && kastDeuvelsVerplaatst < 20)) {
        if (stepArray[0].p == 'box') {
            ...
            if(stepsTaken == 24){
                ...
                document.querySelector('#kastent').emit('tweedeAnimatie');
                document.querySelector('#kastent').emit('tweedeAnimatie2');
                $('#s1').remove();
                $('#s2').remove();
                $('#s3').remove();
                $('#s4').remove();
            ...
            if(stepsTaken >= 20){
                if(stepsTaken == 20){
                    document.querySelector('#kastent').emit('eersteAnimatie');
                }
        ...
        document.querySelector('#kastent').emit('derdeAnimatie');
        document.querySelector('#kastent').emit('derdeAnimatie2');
...
// functie die ervoor zorgt dat kastPlank3 wordt geplaatst als de waarde van 'hand'
// gelijk is aan 'selector'
...
/* functie die word aangeroepen als de gebruiker richt met de cursor op een schroefbox
    vervolgens word de box vervangen met een schroef object */
...
// als de waarde van hand 'p2' (dus als de handhammer is vastgepakt) is dan
// gaat de animatie af van de hammer en word de positie van de schroef veranderd
...
// functie die huidige kastblad vervangt met volgende kastblad
...
/* Als de gebruiker kijkt naar het ikea bord dan word de functies aangeroepen */
...
// functie die de locatie opvraagt om vervolgens te kijken welke ikea het dichtsbijzijnde is
...
// berekent afstand tot dichtsbijzijnde ikea
function afstand() {
  var data = $.getJSON({
    url: 'locaties.JSON',
    async: false
  ...
  // for loop door de json array waar de locaties inzitten
